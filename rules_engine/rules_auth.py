...
class PrivilegeEscalationRule(ThreatRule):
    ...
    check_fields = ["uri_path", "uri_query", "original_message"]
    patterns = [
        r"(?i)((;\s*|\|\||&&|\|)\s*sudo|su|chmod|chown|setcap)\b|(cmd=|exec=|system=).*(sudo|su|chmod|chown)\b|chmod\s+[0-7]*4[0-7]{2}|\bsu\s+-?\s*root\b|bash\s+-p|sh\s+-p|python.*pty\.spawn|/etc/(sudoers|shadow)|setcap\s+|linpeas|linenum|pspy",
    ]
