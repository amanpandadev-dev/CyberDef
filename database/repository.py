...
class FileMetadataRepository:
    ...
    @staticmethod
    def start_processing(session: Session, file_id: str) -> None:
    ...
    @staticmethod
    def mark_failed(session: Session, file_id: str) -> None:
        """Set status to FAILED and record end time."""
        session.execute(
            update(FileMetadataDB)
            .where(FileMetadataDB.file_id == file_id)
            .values(
    ...
    def update_analysis_stats(
        ...
        """Update analysis statistics and mark as PROCESSED."""
        session.execute(
            update(FileMetadataDB)
            .where(FileMetadataDB.file_id == file_id)
            .values(
        ...
        session.flush()
        logger.info(f"Updated analysis stats and marked PROCESSED: {file_id}")
