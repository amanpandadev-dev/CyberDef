...
class NormalizationService:
    ...
    def normalize_event(self, parsed: ParsedEvent) -> NormalizedEvent | None:
        ...
        try:
            ...
            full_url = str(url).strip() if url else None
            uri_path, uri_query = self._extract_uri_parts(full_url)
            ...
            # Identity mapping with fallback for user_id
            username = parsed.username
            if not username and parsed_data:
                username = parsed_data.get("user_id") or parsed_data.get("UserID")
            # Derive deterministic event_id from row_hash so the same log row
            # always maps to the same UUID across runs, fixing deduplication inconsistency.
            event_id = UUID(parsed.row_hash.ljust(32, '0')[:32])
            event = NormalizedEvent(
                ...
                # HTTP fields
                http_method=(
                    str(parsed_data.get("http_method") or parsed_data.get("HTTPMethod")).upper()
                    if (parsed_data.get("http_method") or parsed_data.get("HTTPMethod"))
                    else None
                ),
                http_status=self._safe_int(parsed_data.get("http_status") or parsed_data.get("responsecode")),
                url=full_url,
                uri_path=uri_path,
                uri_query=uri_query,
                user_agent=(
                    parsed_data.get("user_agent")
                    or parsed_data.get("UserAgent")
                    or vendor_data.get("user_agent")
                    or vendor_data.get("UserAgent")
                ),
                referrer=(
                    parsed_data.get("referrer")
                    or parsed_data.get("HTTPReferer")
                    or vendor_data.get("referrer")
                    or vendor_data.get("HTTPReferer")
                ),
                ...
                response_size=self._safe_int(parsed_data.get("response_size") or parsed_data.get("RXLen")),
    ...
    def _extract_uri_parts(self, raw_url: Any) -> Tuple[str | None, str | None]:
        ...
        if "?" not in url and "=" in url and not url.startswith("/") and "://" not in url:
            return None, url
