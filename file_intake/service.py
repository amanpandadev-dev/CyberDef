...
class FileIntakeService:
    ...
    async def start_processing(self, file_id: str) -> None:
        """Lightweight stage tracking: mark file as processing."""
        with get_db_session() as session:
            FileMetadataRepository.start_processing(session, file_id)
        logger.info(f"Marked file as PROCESSING | file_id={file_id}")

    async def mark_failed(self, file_id: str) -> None:
        """Lightweight stage tracking: mark file as failed."""
        with get_db_session() as session:
            FileMetadataRepository.mark_failed(session, file_id)
        logger.info(f"Marked file as FAILED | file_id={file_id}")
